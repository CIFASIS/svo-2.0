...
protected:
  ...
  size_t sparseImageAlignment(const Transformation& T_lastimu_newimu_prior, const bool use_prior);

  size_t projectMapInFrame();

  size_t optimizePose(const Transformation& T_lastimu_newimu_prior, const bool use_prior);
  ...
  /// Get motion prior, expressed in IMU frame.
  virtual bool getMotionPrior(
      const FrameBundlePtr& old_frame,
      const FrameBundlePtr& new_frame,
      const bool use_velocity_in_frame,
      Transformation& T_lastimu_newimu);
};
