#!/bin/bash
#
# Perform several SVO 2.0 operations in a Docker container.
...
if [ $DEV_MODE -eq 1 ] ; then
  docker run -it --rm --net=host -v $CURRENT_DIR/svo_ros:/root/catkin_ws/src/svo_ros/ \
   svo2:ros-kinetic
fi
...
if [ $VIS_MODE -eq 1 ] ; then
  if [ $DETACHED -eq 1 ] ; then
    docker run -d --net=host \
       -v $CURRENT_DIR/svo_ros/calib/:/root/catkin_ws/src/svo_ros/calib \
       -v $CURRENT_DIR/svo_ros/launch/:/root/catkin_ws/src/svo_ros/launch \
       -v $CURRENT_DIR/svo_ros/param/:/root/catkin_ws/src/svo_ros/param \
       svo2:ros-kinetic \
...
if [ $BUILD -eq 1 ] ; then
  docker build --rm --tag svo2:ros-kinetic $CURRENT_DIR
fi
